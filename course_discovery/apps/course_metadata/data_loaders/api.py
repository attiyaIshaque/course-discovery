...
class EcommerceApiDataLoader(AbstractDataLoader):
    ...
    def update_enrollment_code(self, body):
        ...
        if body['stockrecords']:
            ...
            msg = 'Enrollment code product {enrollment_code} has no stockrecords'.format(enrollment_code=title)
            logger.warning(msg)
            return None
        ...
        try:
            ...
            msg = 'A necessary stockrecord field is missing or incorrectly set for enrollment code {code}'.format(
                code=title
            )
            logger.warning(msg)
            return None
        ...
        try:
            ...
            msg = 'Could not find course run {key} while loading enrollment code {code} with sku {sku}'.format(
                key=course_key, code=title, sku=bulk_sku
            )
            logger.warning(msg)
            return None
        ...
        try:
            ...
            msg = 'Could not find currency {code} while loading entitlement {entitlement} with sku {sku}'.format(
                code=currency_code, entitlement=title, sku=bulk_sku
            )
            logger.warning(msg)
            return None
        ...
        try:
            ...
            msg = 'Could not find seat type {type} while loading enrollment code {code} with sku {sku}'.format(
                type=seat_type, code=title, sku=bulk_sku
            )
            logger.warning(msg)
            return None
        ...
        msg = 'Creating enrollment code {enrollment_code} with sku {sku} for partner {partner}'.format(
            enrollment_code=title, sku=bulk_sku, partner=self.partner
        )
        logger.info(msg)
